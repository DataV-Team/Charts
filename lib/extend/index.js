...
var pie = {
  ...
  draw: function draw(_ref2, _ref3) {
    ...
    ctx.lineTo.apply(ctx, _toConsumableArray(connectPoint1));
    ctx.arc(rx, ry, or > 0 ? or : 0, endAngle, startAngle, clockWise);
    ctx.lineTo.apply(ctx, _toConsumableArray(connectPoint2));
    ctx.closePath();
    ctx.stroke();
    ctx.fill();
  }
...
var numberText = {
  ...
  draw: function draw(_ref8, _ref9) {
    ...
    ctx.closePath();
    ctx.strokeText.apply(ctx, [textString].concat(_toConsumableArray(position)));
    ctx.fillText.apply(ctx, [textString].concat(_toConsumableArray(position)));
  }
};
var lineIcon = {
  shape: {
